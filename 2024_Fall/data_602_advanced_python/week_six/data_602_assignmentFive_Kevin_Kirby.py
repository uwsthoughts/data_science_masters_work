...
def exercise13(n):
    ...
    s = pd.Series(np.random.randint(0, n, size=n), index=date_range)
    s.cumsum().plot()
    plt.xlabel('Date')
    plt.ylabel('Cumulative Sum')
    plt.title('Cumulative Sum of Random Integers Over Time')
    plt.show()
...
def exercise14(words):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = pd.DataFrame({'word_length': pd.Series(words).map(lambda word: len(word))})
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df, _, _, _, _ = exercise08()  # Get the DataFrame from exercise08
