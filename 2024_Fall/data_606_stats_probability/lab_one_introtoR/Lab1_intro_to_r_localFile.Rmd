...
**Insert your answer here**
Code for line plot graph of girls baptized data:
```{r plot-girls-vs-year-line-assignment}
ggplot(data = arbuthnot, aes(x = year, y = girls)) + 
  geom_line()
```


There was a sharp decline in the numbers of girls Baptized between 1640 and 1650 that was met with a equal spike in girls being baptized between 1660 and 1665. This suggests dramatic changes in the nature of the religion between 1640 and 1665. Interestingly, in 1665 the trend return to the trajectory it probably would have been on had the dramatic events not occured. The rest of the trends are within the typical ossicilation patterns, although the sharp spike and return back up around 1705 is worth investigating more. Of course, Arthouse was using this data as a proxy for ratio of births between boys and girls but this only works if there's a 1:1 relationship between every child actually born and a baptism record. If a child was born but wasn't baptized then the dataset wouldn't reflect population trends so much as changes in the makeup of the religion. 
...
**Insert your answer here**
Code for line plot graph of ratio of boys baptized over time:
```{r plot-boy_ratio-vs-year}
ggplot(data = arbuthnot, aes(x = year, y = boy_ratio)) + 
  geom_line()
```

A line drawn from the first data point in 1629 to the last data point in 1710 would be a solid negative trend line, showing a noticeable shift happening. However, this is only an overall change from .527 to .518, which isn't that big of a difference overall. Additionally, this data is really noisy in between, with spikes up and down seemingly every year. However, and this is why how you scale your charts matters, is this actually that big of a trends or is the scale warping reality? 
...
**Insert your answer here**
To determine the number of years, I loaded the dataset using:
```{r load present data}
data('present', package='openintro')
```
I then visually inspected the data to see that ther weren't that many rows. I saw the data goes from row 1 with year 1940 down to row 63 with year 2002. I can visually see there's a row for each year, although this approach only works on smaller datasets like this. I would expect there to be 62 total rows between 1940 and 2002 to allow for a row of data for each year. That's exactly the case here. 


Code for getting dimensions of data frame:
```{r glimpse-data-frame}
view(present)
```
Dimensions: 63 rows and 3 columns
...
Adding totals to the dataset:
```{r present-totals}
present <- present %>%
  mutate(total = boys + girls)
```
I used line charts for the boys and girls data from present to compare to Arbuthnot:

Girls chart:
```{r girls-present-line-chart}
ggplot(data = present, aes(x = year, y = girls)) + 
      geom_line()
```

Boys chart:
```{r boys-present-line-chart}
ggplot(data = present, aes(x = year, y = boys)) + 
    geom_line()
```

Addtionally, I used the min and max functions from the lab to get a sense of scale:

Girls:
```{r min-max-girls-present}
present %>%
 summarize(min = min(girls), max = max(girls))
```

Boys:
```{r min-max-boys-present}
present %>%
 summarize(min = min(boys), max = max(boys))
```

Arbuthnot:
boys - min: 2980; max: 8426
girls - min: 2722; max: 7779

Present:
boys - min: 1211684; max: 2186274
girls - min: 1148715; max: 2082052

While the Arthouse dataset has a larger number of years covered (81 versus 62), the raw values in the present dataset are significantly larger than Arthouse's. This makes present not of a similar magnitude for it's magnitude is far larger.
...
Boys ratio line chart:
```{r boy-ratio-present}
ggplot(data = present, aes(x = year, y = boy_ratio)) + 
  geom_line()
```

The boy ratio is also higher in the United States during this later time frame, with the value between ossciliating between .512 and .514, depending on the year. I'm skeptical that the downward trend means much since we are talking about changes in hundredths of a percentage over a 60+ year span. Arthouse's observation would techically holdup but I'm skeptical of the value of that connection without more contextual data. 
...
**Insert your answer here**
```{r graded-arrange-code}
present %>%
  arrange(desc(total))
```
Year with the most number of total births in the United States: 1961 with 4268326 total births.
